...
export default async function DesignersPage() {
  const res = await fetch(
    `${process.env.API_URL}/api/users/getUsers/getDesigners`,
    {
      next: { revalidate: 60 }, // This enables ISR (revalidate every 60 seconds)
    }
  );
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8 px-4">
      <h1 className="text-3xl font-bold text-center mb-8">Designers</h1>
...
// Fetching logic for designers
const fetcher = (url) => fetch(url).then((res) => res.json());

const DesignersPage = () => {
  const { data: users, error } = useSWR(
    "/api/users/getUsers/getDesigners",
    fetcher
  );
